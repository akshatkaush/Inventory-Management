...
from .fpn import BiFPN, FPN
from .backbone.resnet import resnest18
import torch.nn as nn
from .loss import GenTargets, LOSS, coords_fmap2orig
import torch
from .config import DefaultConfig
from .backbone.rexnet import ReXNetV1
...
class FCOS(nn.Module):
    def __init__(self, config=None):
        ...
        self.backbone = ReXNetV1(width_mult=1.0, depth_mult=1.0)
        # self.c1 = nn.Conv2d(16, 64, kernel_size=1)
        # self.c2 = nn.Conv2d(32, 64, kernel_size=1)
        # self.c3 = nn.Conv2d(64, 64, kernel_size=1)
        # self.c4 = nn.Conv2d(128, 64, kernel_size=1)

        self.fpn = nn.Sequential(
            *[
                BiFPN(256, [320, 640, 1280], True if _ == 0 else False, attention=True)
                for _ in range(2)
            ]
        )
        # self.fpn = FPN(features=256)
        self.head = ClsCntRegHead(
            256, config.class_num, config.use_GN_head, config.cnt_on_reg, config.prior
        )
        self.config = config
...
if __name__ == "__main__":
    ...
    model([batch_imgs, batch_boxes, batch_classes])