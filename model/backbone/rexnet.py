...
class ReXNetV1(nn.Module):
    def __init__(
        ...
        in_channels_for_fpn = {7: 61, 13: 128}
        pen_channels_for_fpn = {7: int(320 * width_mult), 13: int(640 * width_mult)}
        ...
        ConvBNSwish(
            c1,
            in_channels_for_fpn[7],
            pen_channels_for_fpn[7],
        )
        c2 = []
        ConvBNSwish(
            c2,
            in_channels_for_fpn[13],
            pen_channels_for_fpn[13],
        )

        c3 = []
        ConvBNSwish(c3, c, pen_channels)
        self.c1 = nn.Sequential(*c1)
        self.c2 = nn.Sequential(*c2)
        self.c3 = nn.Sequential(*c3)
        self.mapping = {7: self.c1, 13: self.c2, 18: self.c3}
    ...
    def forward(self, x):
        ...
        for idx, l in enumerate(self.features):
            ...
            #            print(idx, x.shape)
            if idx in [7, 13, 18]:
                output.append(self.mapping[idx](x))

        return output[0], output[1], output[2]


if __name__ == "__main__":
    model = ReXNetV1(width_mult=1, depth_mult=1.0)
    model(torch.zeros((2, 3, 320, 1408)))
